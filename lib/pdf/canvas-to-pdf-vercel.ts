/**
 * Vercel-Compatible Canvas-to-PDF using @sparticuz/chromium-min
 *
 * This is a fork of canvas-to-pdf-simple.ts that works on Vercel serverless.
 * Uses @sparticuz/chromium-min instead of puppeteer for browser automation.
 *
...
// Chromium binary URL (hosted on GitHub releases for reliability)
const CHROMIUM_EXECUTABLE_URL =
  'https://github.com/nickvidal/chromium/releases/download/v121.0.6167.85/chromium-v121.0.6167.85-pack.tar'
...
async function getBrowserInstance(): Promise<Browser> {
  ...
  console.log('🚀 [PDF-Vercel] Launching Chromium via @sparticuz/chromium-min...')

  browserInstance = await puppeteer.launch({
    args: chromium.args,
    executablePath: await chromium.executablePath(CHROMIUM_EXECUTABLE_URL),
  ...
  browserRefCount = 1
  console.log('✅ [PDF-Vercel] Browser launched')
  return browserInstance
}
...
async function releaseBrowserInstance(): Promise<void> {
  ...
  if (browserRefCount <= 0 && browserInstance) {
    ...
    browserRefCount = 0
    console.log('✅ [PDF-Vercel] Browser closed')
  }
}
...
async function downloadCanvasImages(canvasJSON: any): Promise<any> {
  ...
  if (images.length > 0) {
    console.log(`📥 [PDF-Vercel] Downloading ${images.length} images...`)
    await Promise.all(
    ...
    console.log('✅ [PDF-Vercel] Images downloaded')
  }
...
export async function convertCanvasToPDF(
  ...
  if (
    ...
    console.log('📌 [PDF-Vercel] Old signature detected (backwards compatibility mode)')
  ...
  console.log(`🖼️ [PDF-Vercel] Converting ${actualBackCanvas ? 'front + custom back' : 'front + blank back'} for ${actualRecipientData.name || 'recipient'}...`)

  const browser = await getBrowserInstance()

  try {
    const format = getFormat(actualFormatType)
    console.log(`📐 [PDF-Vercel] Format: ${format.widthPixels}×${format.heightPixels}px`)

    // RENDER FRONT PAGE (required)
    console.log('🎨 [PDF-Vercel] Rendering front page...')
    ...
    console.log('✅ [PDF-Vercel] Front page rendered')

    // RENDER BACK PAGE (custom or blank)
    let backImageBase64: string
    if (actualBackCanvas) {
      console.log('🎨 [PDF-Vercel] Rendering custom back page...')
      ...
      console.log('✅ [PDF-Vercel] Custom back page rendered')
    } else {
      console.log('📄 [PDF-Vercel] Creating blank back page (PostGrid address block)...')
      backImageBase64 = createBlankPageImage(format.widthPixels, format.heightPixels)
      console.log('✅ [PDF-Vercel] Blank back page created')
    }

    // CREATE PDF WITH BOTH PAGES
    console.log('📄 [PDF-Vercel] Assembling 2-page PDF...')
    ...
    console.log(`📏 [PDF-Vercel] Dimensions: ${pdfWidth}" × ${pdfHeight}" (trim: ${format.widthInches}" × ${format.heightInches}", bleed: ${format.bleedInches}")`)
    ...
    console.log(`✅ [PDF-Vercel] 2-page PDF created (front + ${actualBackCanvas ? 'custom back' : 'blank back'})`)

    const pdfBuffer = Buffer.from(pdf.output('arraybuffer'))
    console.log(`✅ [PDF-Vercel] Complete: ${(pdfBuffer.length / 1024).toFixed(2)} KB (2 pages)`)
    ...
    console.error('❌ [PDF-Vercel] Failed:', error)
    throw error
  } finally {
